"""
############################################################
## Test Case ID : QCL-3139
## Title        : Entra ID SSO Login Validation on IGEL OS
##
## Description:
# Validates Entra ID SSO authentication on IGEL OS and ensures
...
def QCL_3139_step1(api_config, browser):
    """ 
    ...
    try:
        ...
        profile_sso = api_config["profile_sso_entraid"]
        ...
        if already_assigned:
            ...
            browser.reload_page()
            time.sleep(8)
            return click.is_text_present_on_screen("Microsoft", refresh_before_check=True)
        else:
...
def QCL_3139_step2(api_config,browser):
    """ 
    ...
    try:
        entraid_username = api_config["entraid_username"]
        entraid_password = api_config["entraid_password"]

        browser.reload_page()
        if click.click_text_on_screen("email", refresh_after_click=True):
            pyautogui.write(entraid_username, interval=0.05)
            time.sleep(1)
            click.click_text_on_screen("next", refresh_after_click=False)
            time.sleep(2)
        pyautogui.write(entraid_password, interval=0.05)
        time.sleep(1)
...
def QCL_3139_step4(browser):
    ...
    """
    logger.info("Starting Step 4: Verify login works with internet")
    try:
        ssh = get_ssh()
        if ssh:
...
def QCL_3139_step5(api_config,browser):
    """ 
        ...
        Returns: 
            bool: True if the password screen is shown, else False. 
    """
    logger.info("Starting Step 5: Enable password screenlock")
    try:
        entraid_password = api_config["entraid_password"]
        ssh = get_ssh()
        browser.reload_page()
        ...
        pyautogui.write(entraid_password)
        ...
        logger.error(f"Exception in step5_password_enable_screenlock: {e}")
        return False


@allure.step("Step 6: Disable password screenlock")
def QCL_3139_step6(api_config, browser):
    """ 
    ...
    """
    logger.info("Starting Step 6: Disable password screenlock")
    try:
        entraid_username = api_config["entraid_username"]
        entraid_password = api_config["entraid_password"]
        ums = UMS(ums_cred["base_url"], ums_cred["username"], ums_cred["password"])
        device_details = ums.get_vm_details(device_cred["hostname"])
        profile_disabled = api_config["profile_sso_disabled_entraid"]
        ...
        browser.reload_page()
        time.sleep(8)
        click.click_text_on_screen("email", refresh_after_click=True)
        pyautogui.write(entraid_username, interval=0.05)
        click.click_text_on_screen("next", refresh_after_click=False)
        time.sleep(2)
        pyautogui.write(entraid_password, interval=0.05)
...
def QCL_3139_step7(api_config, browser):
    """ 
        ...
        Returns: 
            ...
            is accessible, else False. 
    """
    logger.info("Starting Step 7: Verify auto-login")
    try:
        ums = UMS(ums_cred["base_url"], ums_cred["username"], ums_cred["password"])
        device_details = ums.get_vm_details(device_cred["hostname"])
        profile_auto = api_config["profile_sso_autologin_entraid"]
...
def QCL_3139_step8( browser):
    """ 
        Opens the browser on the remote device via SSH and navigates 
        to the Azure portal, then verifies that the expected user 
        element is visible. 
        Returns: 
...
def clean_up(api_config, browser):
    ...
    try:
        ...
        api.set_bearer(bearer_token)
        ssh = get_ssh()
        for profile_key in ["profile_sso_entraid", "profile_sso_disabled_entraid", "profile_sso_autologin_entraid"]:
            browser.reload_page()
...
def test_QCL_3139_step1(api_config, browser_instance):
    assert QCL_3139_step1(api_config, browser_instance)

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step2(api_config,browser_instance):
    assert QCL_3139_step2(api_config,browser_instance)

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step3(browser_instance):
    assert QCL_3139_step3(browser_instance) 

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step4(browser_instance):
    assert QCL_3139_step4(browser_instance)

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step5(api_config,browser_instance):
    assert QCL_3139_step5(api_config,browser_instance)

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step6(api_config, browser_instance):
    assert QCL_3139_step6(api_config, browser_instance)

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step7(api_config, browser_instance):
    assert QCL_3139_step7(api_config, browser_instance)

@allure.feature("SSO Validation")
@allure.severity(allure.severity_level.CRITICAL)
def test_QCL_3139_step8(browser_instance):
    assert QCL_3139_step8(browser_instance)
